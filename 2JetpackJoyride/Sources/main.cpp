...
int main() {
    ...
    while (WindowShouldClose() == false) {
        ...
        if (currentScreen == MENU) {
            ...
            camera.target = (Vector2){player.posX - 1000, screenHeight / 2.0f};
            ...
            if (!coins.empty()) {
                coins.erase(
                    remove_if(coins.begin(), coins.end(), [&](const Coin &coin) {
                    // Check if the coin is sufficiently far behind the player
                    return coin.posX < player.posX - screenWidth / 10.0f;
                }),
                coins.end()
                );
            }   
            ...
            for (int i = 0; i < roof.size(); i++) {
                ...
                DrawTextureEx(floorTexture, Vector2{floor[i].posX, floor[i].posY}, 0, 2, WHITE);
                DrawTextureEx(roofTexutre, Vector2{roof[i].posX, roof[i].posY}, 0, 2, WHITE);
            }
